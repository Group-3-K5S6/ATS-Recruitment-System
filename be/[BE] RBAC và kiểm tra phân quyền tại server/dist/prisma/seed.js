...
const roles_1 = require("../src/rbac/roles");
const permissions_1 = require("../src/rbac/permissions");
const role_permissions_1 = require("../src/rbac/role-permissions");
...
async function runSeed() {
    ...
    for (const roleName of roles_1.ALL_ROLES) {
        const role = await prisma.role.upsert({
            where: { name: roleName },
            update: { description: roles_1.ROLE_DESCRIPTIONS[roleName] },
            create: {
                name: roleName,
                description: roles_1.ROLE_DESCRIPTIONS[roleName],
            },
        });
    ...
    for (const perm of permissions_1.ALL_PERMISSIONS) {
    ...
    for (const roleName of roles_1.ALL_ROLES) {
        ...
        const permissionsForRole = role_permissions_1.ROLE_PERMISSIONS[roleName] || [];
    ...
    const defaultPasswordHash = await (0, password_1.hashPassword)('Password123!');
    async function seedUser(email, fullName, roleType, deptId) {
    ...
    const admin = await seedUser('admin@ats.local', 'System Administrator', roles_1.RoleType.ADMIN);
    const hrManager = await seedUser('hr_manager@ats.local', 'HR Manager User', roles_1.RoleType.HR_MANAGER, hrDept.id);
    const recruiter1 = await seedUser('recruiter1@ats.local', 'Recruiter One', roles_1.RoleType.RECRUITER, hrDept.id);
    const recruiter2 = await seedUser('recruiter2@ats.local', 'Recruiter Two', roles_1.RoleType.RECRUITER, hrDept.id);
    const hmEng = await seedUser('hiring_manager_eng@ats.local', 'Engineering Lead', roles_1.RoleType.HIRING_MANAGER, engDept.id);
    const hmMkt = await seedUser('hiring_manager_mkt@ats.local', 'Marketing Lead', roles_1.RoleType.HIRING_MANAGER, mktDept.id);
    const interviewer1 = await seedUser('interviewer1@ats.local', 'Senior Dev Interviewer', roles_1.RoleType.INTERVIEWER, engDept.id);
    const interviewer2 = await seedUser('interviewer2@ats.local', 'Tech Lead Interviewer', roles_1.RoleType.INTERVIEWER, engDept.id);
    const approver = await seedUser('approver@ats.local', 'VP Approver', roles_1.RoleType.APPROVER, hrDept.id);
    const candidateUser1 = await seedUser('candidate1@ats.local', 'Nguyễn Văn Ứng Viên 1', roles_1.RoleType.CANDIDATE);
    const candidateUser2 = await seedUser('candidate2@ats.local', 'Trần Thị Ứng Viên 2', roles_1.RoleType.CANDIDATE);
